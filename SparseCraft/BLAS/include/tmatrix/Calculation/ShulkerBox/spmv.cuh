...
__device__ __forceinline__ MatValue spm_dv(const Tile* A_tiles, const char*A_data, const MatIndex*A_colind, const MatValue *x, MatValue *y, int&lane_id, MatIndex&start_aj, MatIndex&end_aj, MatValue*warp_x, MatValue*warp_y)
{
    int vwarp_id = lane_id >> 1, hwarp_id = lane_id >> 4;
    int vlane_id = lane_id & 1, hlane_id = lane_id & 15;
    MatValue sumsum = 0;
    for (MatIndex Aj = start_aj; Aj < end_aj; ++Aj)
    {
        TileIndex *bits = (TileIndex*)(A_data + A_tiles[Aj].bits_off);
        const MatValue *A = (MatValue*)(A_data + A_tiles[Aj].bits_off + A_tiles[Aj].bitslen * 16), *dx = x + (A_colind[Aj] << 4);
        ...
        switch (A_tiles[Aj].fmt)
        {
            ...
            case CSR:
                {
                    ...
                    TileIndex *colidx = bits + TILE_N + 1;
            ...
            case HYB:
                {
                    ...
                    MatIndex coo_start = (min_row_len * 16 + 1) / 2 + 2;
            ...
            case DRW:
                {
                    ...
                    MatValue rx = dx[hlane_id];
                    for (int j = hwarp_id; j < cnt; j += 2)
                    {
                        MatValue sum = A[j * 16 + hlane_id] * rx;
            ...
            case DNS:
                {
                    MatValue sum = 0, rx = lane_id < 16? dx[hlane_id]: 0;
