...
def workload(task, mtx, process, *args):
    print(f'./dist/{task} "{mtx_dir_path}/{mtx}" {" ".join([str(i) for i in args])}')
    st, rt = external_exec(f'./dist/{task} "{mtx_dir_path}/{mtx}" {" ".join([str(i) for i in args])}', without_output=True)
    if not st:
        rt = rt.splitlines()[-1]
