...
int lb_spmv_coo_style(BaseMatrix*m, MatIndex **blkcoostylerowidx, MatIndex**blkcoostylerowidx_colstart, MatIndex**blkcoostylerowidx_colstop)
{
    ...
    echo(debug, "rowblkblockcnt: %d", rowblkblockcnt);
    cudaCheckError(cudaMalloc(blkcoostylerowidx, sizeof(MatIndex) * rowblkblockcnt));
    cudaCheckError(cudaMemcpy(*blkcoostylerowidx, blkcoostylerowidx_tmp, sizeof(MatIndex) * rowblkblockcnt, cudaMemcpyHostToDevice));
    cudaCheckError(cudaMalloc(blkcoostylerowidx_colstart, sizeof(MatIndex) * rowblkblockcnt));
    cudaCheckError(cudaMemcpy(*blkcoostylerowidx_colstart, blkcoostylerowidx_colstart_tmp, sizeof(MatIndex) * rowblkblockcnt, cudaMemcpyHostToDevice));
    cudaCheckError(cudaMalloc(blkcoostylerowidx_colstop, sizeof(MatIndex) * rowblkblockcnt));
    cudaCheckError(cudaMemcpy(*blkcoostylerowidx_colstop, blkcoostylerowidx_colstop_tmp, sizeof(MatIndex) * rowblkblockcnt, cudaMemcpyHostToDevice));
