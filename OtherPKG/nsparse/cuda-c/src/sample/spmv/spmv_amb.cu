...
void spmv_amb(sfCSR *csr_mat, real *x, real *y, sfPlan *plan)
{
    ...
    csr_memcpy(csr_mat);
    cudaMalloc((void **)&d_x, sizeof(real) * (csr_mat->N + MAX_BLOCK_SIZE));
    cudaMalloc((void **)&d_y, sizeof(real) * (csr_mat->M + WARP));
    cudaMemcpy(d_x, x, sizeof(real) * csr_mat->N, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(y, d_y, sizeof(real) * csr_mat->M, cudaMemcpyDeviceToHost);
