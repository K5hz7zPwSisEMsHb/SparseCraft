...
int binary_read_csr(int *row, int *col, int *nnz, int **row_ptr, int **col_idx, MAT_VAL_TYPE **val, char *filename)
{
    FILE *fp = NULL;
    fp = fopen(filename, "r+");
    if (fp == NULL)
    ...
    *val = (MAT_VAL_TYPE *)malloc(sizeof(MAT_VAL_TYPE) * (*nnz));
    if ((*nnz) != fread(*val, sizeof(MAT_VAL_TYPE), (*nnz), fp))
    {
...
void binary_write_csr(int row, int col, int nnz, int *row_ptr, int *col_idx, MAT_VAL_TYPE *val, char *filename)
{
    ...
    if (fp != NULL)
    {
        ...
        fwrite(val, sizeof(MAT_VAL_TYPE), nnz, fp);
    }
    else
    {
        INFO_LOG("open error\n");
...
void read_Dmatrix_convert(int *row, int *col, int *nnz, int **row_ptr, int **col_idx, MAT_VAL_TYPE **val, int *isSymmeticeR, char *filename)
{
...
void read_Dmatrix(int *row, int *col, int *nnz, int **row_ptr, int **col_idx, MAT_VAL_TYPE **val, int *isSymmeticeR, char *filename)
{
