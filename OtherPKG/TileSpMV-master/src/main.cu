...
int main(int argc, char ** argv)
{
    ...
    read_Dmatrix(&rowA, &colA, &nnzA, 
                &csrRowPtrA, &csrColIdxA, &csrValA, &isSymmetricA, filename);
...
//run GPU TilespMV
    ...
    double total_byte1 = mem_footprint + (colA + rowA) * sizeof(MAT_VAL_TYPE);
    double total_byte2 = mem_footprint + (nnzA + rowA) * sizeof(MAT_VAL_TYPE);
    double bandwidth1 = total_byte1 * 1.0 / (spmv_time * 1.0e6);
    double bandwidth2 = total_byte2 * 1.0 / (spmv_time * 1.0e6);
    mem_footprint = mem_footprint / 1.0e6;    //in MB
    if (spmv_gflops > 0) printf("%lf,%lf\n", convert_time, spmv_gflops);
    else printf("0\n");
