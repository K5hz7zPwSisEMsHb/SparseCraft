...
void ready(int argc, char **argv)
{
	...
	fp = fopen(argv[1], "r");
	fgets(buf, 300, fp);
	FILE *fout = fopen("ASpT-results-128.csv", "a");
	if (fout == NULL)
...
__global__
	...
	void
	spmv_kernel64_sparse_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel64_sparse_v2l(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
	int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5); // + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR + 1)) + lane;
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel64_sparse_v2h(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *special, int *special2)
{
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel64_ssparse(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
	int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5); // + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR + 1)) + lane;
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel64_dense_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *baddr, int *saddr)
{
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR + 1)) + lane;
	int offset2 = offset + MFACTOR;
...
__global__ void cal_vari(int nr, double avg, int *mcsr_cnt, int *mcsr_e, double *vari, int *special_bb)
{
	...
	if ((threadIdx.x & 31) == 0)
	{
		...
		if (i2 > 0)
			atomicAdd(&special_bb[((blockIdx.x * blockDim.x + threadIdx.x) >> 5) & 127], i2);
	}
}
...
#define MCSR_CNT_TBSIZE (1024)
	...
	for (int ldc = sc; ldc <= sc; ldc *= 2)
	{
		...
		// process
		dim3 s_gridsize(nr / SBF, 1, CEIL(ldc, MFACTOR * 2));
		dim3 s_blocksize(SBSIZE, 1, 1);
		dim3 ss_gridsize(nr / SBF, 1, CEIL(ldc, MFACTOR * 2));
		dim3 ss_blocksize(SBSIZE, 1, 1);
		dim3 d_gridsize(num_dense, 1, CEIL(ldc, MFACTOR * 2));
		dim3 d_blocksize(DBSIZE, 1, 1);
		dim3 s_gridsizeh(special_p, 1, CEIL(ldc, MFACTOR * 2));
		...
		if (ne / nc < 6 && vari < 40)
		{
			...
			{
				// kernel fun
				spmv_kernel64_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				cudaDeviceSynchronize();
			}
		}
		else if (vari < 200)
		{
			...
			{
				// kernel fun
				spmv_kernel64_sparse_v2<<<s_gridsize, s_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				spmv_kernel64_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
				cudaDeviceSynchronize();
			}
		}
		else
		{
			...
			{
				// kernel fun
				spmv_kernel64_sparse_v2l<<<s_gridsize, s_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				spmv_kernel64_sparse_v2h<<<s_gridsizeh, s_blocksizeh, 0, stream3>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _special, _special2);
				spmv_kernel64_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
				cudaDeviceSynchronize();
			}
		}
		...
		if (fout == NULL)
			printf("Writing results fails.\n");
		fprintf(fout, "%i,%i,%i,%i,%f,%f\n", nr, nc, ne, ldc, time_cuda_spmm_base, gflops);
		fclose(fout);
		fprintf(fpo, "%f,", (double)ITER * (double)ne * 2 * ldc / tot_ms / 1000000);
