...
#define GPRINT(x, y)                                               \
	int *tt0 = (int *)malloc(sizeof(int) * (y));                   \
	fprintf(stderr, "\n");                                         \
	cudaMemcpy(tt0, x, sizeof(int) * (y), cudaMemcpyDeviceToHost); \
	for (int i = 0; i < (y); i++)                                  \
		fprintf(stderr, "%d ", tt0[i]);                            \
	fprintf(stderr, "\n");                                         \
	free(tt0);

#define GPRINT2(x, y)                                              \
	int *tt1 = (int *)malloc(sizeof(int) * (y));                   \
	fprintf(stderr, "\n");                                         \
	cudaMemcpy(tt1, x, sizeof(int) * (y), cudaMemcpyDeviceToHost); \
	for (int i = 0; i < (y); i++)                                  \
		fprintf(stderr, "%d ", tt1[i]);                            \
	fprintf(stderr, "\n");                                         \
	free(tt1);
...
void ready(int argc, char **argv)
{
	...
	fp = fopen(argv[1], "r");
	fgets(buf, 300, fp);
	FILE *fout = fopen("ASpT-results-32.csv", "a");
	if (fout == NULL)
...
__global__
	...
	void
	spmv_kernel32_sparse_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
	int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5); // + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR)) + lane;
	int i, j;
	...
	if (interm3 < loc2)
	{
		FTYPE v = __shfl_sync(0xFFFFFFFF, buf2, jj, MFACTOR);
		int i = __shfl_sync(0xFFFFFFFF, buf, jj, MFACTOR);
		if (offset < sc)
		{
			r += v * vin[i + offset];
		}
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel32_sparse_v2l(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
	int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5); // + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR)) + lane;
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel32_sparse_v2h(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *special, int *special2)
{
	...
	__syncthreads();
	if (threadIdx.x < (SPBSIZE >> 1) && offset < sc)
	{
		sout[threadIdx.x] += sout[threadIdx.x + (SPBSIZE >> 1)];
	}
	__syncthreads();

	if (threadIdx.x < 32 && offset < sc)
	{
		r = 0.0f;
#pragma unroll
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel32_ssparse(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout)
{
	int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5); // + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR)) + lane;
	...
	if (loc2 - loc1 < 32)
	{
		...
		{
			FTYPE v = __shfl_sync(0xFFFFFFFF, buf2, jj, MFACTOR);
			int i = __shfl_sync(0xFFFFFFFF, buf, jj, MFACTOR) * sc;
			if (offset < sc)
			{
				r += v * vin[i + offset];
			}
...
__global__
	//__launch_bounds__(BSIZE, 2048/BSIZE)
	void
	spmv_kernel32_dense_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *baddr, int *saddr)
{
	int lane = (threadIdx.x & (MFACTOR - 1));
	int offset = (blockIdx.z << (LOG_MFACTOR)) + lane;
	int loop, i, j;
...
__global__ void dense_block_detect(int *csr_v, int *mcsr_chk, int *csr_e0, int *flag)
{
	...
	for (i = lb + threadIdx.x; i < ub; i += blockDim.x)
	{
		...
		if (scr_pad[key] < THRESHOLD)
			atomicAdd(&scr_pad[key], 1);
	}
	__syncthreads();
	int r = 0;
	for (i = threadIdx.x; i < SC_SIZE; i += blockDim.x)
	{
		if (scr_pad[i] >= THRESHOLD)
			r++;
	}
	__syncthreads();
	...
	if (threadIdx.x == 0)
	{
		...
		if (r >= MIN_OCC)
		{
			...
			if (flag[blockIdx.x & 127] == 0)
				flag[blockIdx.x & 127] = 1;
		}
	}
}
...
__global__ void cal_vari(int nr, double avg, int *mcsr_cnt, int *mcsr_e, double *vari, int *special_bb)
{
	...
	if ((threadIdx.x & 31) == 0)
	{
		...
		if (i2 > 0)
			atomicAdd(&special_bb[((blockIdx.x * blockDim.x) >> 5) & 127], i2);
	}
}
...
#define MCSR_CNT_TBSIZE (1024)
	...
	// _csr_v = (int) x (nr + 1)
	// _csr_e = (int) x ne + 256
	// _csr_ev = (float) x ne + 256
	// _mcsr_cnt = (int) x (npanel + 1)
	// _mcsr_e = (int) x ne + 256
	// _mcsr_list = (int) x ne + 256

	int memory_footprint = 4 * (nr + 1) + 4 * ne + 256 + 4 * ne +
						   256 + 4 * (npanel + 1) + 4 * ne + 256 + 4 * ne + 256;
	for (int ldc = sc; ldc <= sc; ldc *= 2)
	{
		cudaMemset(_vout, 0, sizeof(FTYPE) * nr * ldc);
		// process
		dim3 s_gridsize(nr / SBF, 1, CEIL(ldc, MFACTOR));
		dim3 s_blocksize(SBSIZE, 1, 1);
		dim3 ss_gridsize(nr / SBF, 1, CEIL(ldc, MFACTOR));
		dim3 ss_blocksize(SBSIZE, 1, 1);
		dim3 d_gridsize(num_dense, 1, CEIL(ldc, MFACTOR));
		dim3 d_blocksize(DBSIZE, 1, 1);
		dim3 s_gridsizeh(special_p, 1, CEIL(ldc, MFACTOR));
		dim3 s_blocksizeh(SPBSIZE, 1, 1);

		if (ne / nc < 6 && vari < 40)
		{
			for (int ik = 0; ik < WRAMUP; ik++)
			{
				// kernel fun
				spmv_kernel32_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				cudaDeviceSynchronize();
			}
		}
		else if (vari < 200)
		{
			for (int ik = 0; ik < WRAMUP; ik++)
			{
				spmv_kernel32_sparse_v2<<<s_gridsize, s_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
				cudaDeviceSynchronize();
			}
		}
		else
		{
			for (int ik = 0; ik < WRAMUP; ik++)
			{
				spmv_kernel32_sparse_v2l<<<s_gridsize, s_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				spmv_kernel32_sparse_v2h<<<s_gridsizeh, s_blocksizeh, 0, stream3>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _special, _special2);
				spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr);
				cudaDeviceSynchronize();
			}
		}
		...
		if (ne / nc < 6 && vari < 40)
		{
			for (int ik = 0; ik < ITER; ik++)
			{
				...
				cudaEvent_t sparse_start;
				cudaEvent_t sparse_end;
				cudaEventCreate(&sparse_start);
				cudaEventCreate(&sparse_end);
				cudaEventRecord(sparse_start, 0);
				spmv_kernel32_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(ldc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout);
				cudaDeviceSynchronize();
				cudaEventRecord(sparse_end, 0);
				cudaEventSynchronize(sparse_end);
				cudaEventElapsedTime(&sparse_ms, sparse_start, sparse_end);
				cudaEventDestroy(sparse_start);
				cudaEventDestroy(sparse_end);
				time_cuda_spmm_base += sparse_ms;
			}
		}
		else if (vari < 200)
		{
		...
		// fprintf(stdout, "%f,%f,", tot_ms,(double)ne*2*ldc/tot_ms/1000000);
		// fprintf(stdout, "%f,%f,", (double)ITER*(double)ne*2*ldc/tot_ms/1000000,(double)ptot_ms/tot_ms);
		time_cuda_spmm_base /= ITER;
		double gflops = 2 * (double)ne * 1.0e-6 / time_cuda_spmm_base * ldc;
		FILE *fout = fopen("ASpT-results-32.csv", "a");
		if (fout == NULL)
			printf("Writing results fails.\n");
		fprintf(fout, "%d,%i,%i,%i,%i,%f,%f\n", memory_footprint, nr, nc, ne, ldc, time_cuda_spmm_base, gflops);
		fclose(fout);
		fprintf(fpo, "%f,", (double)ITER * (double)ne * 2 * ldc / tot_ms / 1000000);

		cudaMemcpy(vout, _vout, sizeof(FTYPE) * nr * ldc, cudaMemcpyDeviceToHost);
...
// #define VALIDATE
		...
		for (i = 0; i < gold_ne; i++)
		{
			for (j = 0; j < ldc; j++)
			{
				vout_gold[gold_temp_v[i].row * ldc + j] += vin[ldc * gold_temp_v[i].col + j] * gold_temp_v[i].val;
			}
		}
		// double x1=0, x2=0;
		int num_diff = 0;
		for (i = 0; i < nr * ldc; i++)
		{
			...
			{
				// printf("(%d %lf %lf) ", i, vout_gold[i], vout[i]);
		...
		printf("num_diff : %d\n", num_diff);
		fprintf(fpo, "%f,", (double)num_diff / (nr * ldc) * 100);
...
#ifdef RUNSINGLE
	// process
	dim3 s_gridsize(nr / SBF, 1, CEIL(sc, MFACTOR));
	dim3 s_blocksize(SBSIZE, 1, 1);
	dim3 ss_gridsize(nr / SBF, 1, CEIL(sc, MFACTOR));
	dim3 ss_blocksize(SBSIZE, 1, 1);
	dim3 d_gridsize(num_dense, 1, CEIL(sc, MFACTOR));
	dim3 d_blocksize(DBSIZE, 1, 1);
	dim3 s_gridsizeh(special_p, 1, CEIL(sc, MFACTOR));
	dim3 s_blocksizeh(SPBSIZE, 1, 1);
	printf("CEIL(sc, MFACTOR):%d\n", CEIL(sc, MFACTOR));
	...
	time_cuda_spmm_base /= ITER;
	double gflops = 2 * (double)ne * 1.0e-6 / time_cuda_spmm_base * sc;
	FILE *fout = fopen("ASpT-results-32.csv", "a");
	if (fout == NULL)
		printf("Writing results fails.\n");
	fprintf(fout, "%i,%i,%i,%i,%f,%f\n", nr, nc, ne, sc, time_cuda_spmm_base, gflops);
	fclose(fout);
