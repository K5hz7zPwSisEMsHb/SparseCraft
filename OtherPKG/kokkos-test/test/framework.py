...
def workload(device, task, mtx, process, *args):
    st, rt = external_exec(f'export CUDA_VISIBLE_DEVICES={device} && ./build/{task} "{mtx_dir_path}/{mtx}" {" ".join([str(i) for i in args])}', without_output=True)
    if not st:
        rt = rt.splitlines()[-1]
...
def start_framework(device, task, ls, odf, csv, *args):
    ...
    try:
        ...
        while ls:
            mtx = ls.pop(0)
            workload(device, task, mtx, (process, task_id), *args)
